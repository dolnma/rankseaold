...
document.getElementsByClassName("flash_text")[i].textContent = i+1;
  ...
  function callouts_on() {
    document.getElementById("map_image").setAttribute('href', "/wp-content/themes/ranksea/nades/assets/maps/de_cache_callouts.png");
  }
  function callouts_off() {
    document.getElementById("map_image").setAttribute('href', "/wp-content/themes/ranksea/nades/assets/maps/de_cache.png");
  }
...
(adsbygoogle = window.adsbygoogle || []).push({});
    ...
    <svg id = "map" viewBox="0 0 1123 850" style="cursor:default">
      <image id="map_image" style="pointer-events:none" height="770" width="1024" y="0" x="0" class="map-image" preserveAspectRatio="none" href="/wp-content/themes/ranksea/nades/assets/maps/de_cache.png"></image>
              <?php
                
                require_once 'popup/dbconfig.php';
                
                $query = "SELECT n.id, n.mode, n.title, n.side, n.line_x1, n.line_y1, n.line_x2, n.line_y2, n.type, n.middle_positions, no.name AS name, no.value AS value FROM nades n JOIN nades_options no ON no.id = n.mode";
                $stmt = $DBcon->prepare( $query );
                $stmt->execute();
                while($row=$stmt->fetch(PDO::FETCH_ASSOC)){
...
<script src="/wp-content/themes/ranksea/js/popup.js" type="text/javascript"></script>
<?php
get_footer();
